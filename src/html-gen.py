...
def main(json_data):
  ...
  for page in json_data['pages']:
    ...
    if 'header' in page['content']:
      items = sorted(page['content']['header']) # Sort JSON elements to prevent them from being inserted in random order
      for item in items:#page['content']['header']:
        file = os.path.normpath(os.path.join(assets_path, page['content']['header'][item]))
    ...
    if 'head' in page['content']:
      ...
      for item in items:#page['content']['head']:
        file = os.path.normpath(os.path.join(assets_path, page['content']['head'][item]))
    ...
    if 'body' in page['content']:
      ...
      for item in items:#page['content']['body']:
        file = os.path.normpath(os.path.join(assets_path, page['content']['body'][item]))
...
def get_datetime_with_offset():
  ...
  utc_offset_seconds = datetime.datetime.fromtimestamp(epochtime) - datetime.datetime.utcfromtimestamp(epochtime)
